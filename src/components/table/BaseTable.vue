...
const selectableRowKeys = computed(() =>
  props.data
    ...
    .filter(({ row, index }) => !props.selectableRow || props.selectableRow(row, index))
    .map(({ row, index }) => getRowKey(row, index))
)

const isAllSelected = computed(() => {
  if (!selectableRowKeys.value.length) return false
  return selectableRowKeys.value.every((key) => selectedKeys.value.includes(key))
})
  ...
  <div class="overflow-x-auto rounded-lg border border-gray-200">
    <table class="min-w-full divide-y divide-gray-200">
      ...
      >
        ...
        <template v-for="column in columns" #[`header-${column.key}`]="slotProps">
